...
func NewPortfolioUseCase(logger logging.Logger, portfolioRepo portfolio.Repository) *PortfolioUseCase {
...
func (p *PortfolioUseCase) Update(ctx context.Context, id int) error {
	...
	config, err := investgo.LoadConfig("internal/portfolio/config.yaml")
	if err != nil {
		// TODO: return error
	...
	for i := 0; i < len(tinkoffAccounts.Accounts); i++ {
		...
		if err != nil {
			// TODO: return error
			p.logger.Fatalf("get portfolio error: %v", err)
		}
...
func (p *PortfolioUseCase) Init(ctx context.Context, token string) (int, error) {
	id, err := p.portfolioRepo.Init(ctx, token)
	if err != nil {
		// TODO: error
		return -1, err
	}
...
func (tuc *TestUseCase) Ha(ctx context.Context, logger logging.Logger) error {
	...
	if err != nil {
		// TODO: return error
		logger.Fatalf("get portfolio error: %v", err)
	}
